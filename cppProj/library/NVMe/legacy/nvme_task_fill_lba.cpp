...
//---------------------------------------------------------------------------
void
NVMeTaskFillLba::pre_fill_after_issue(IoExPlan *plan)
{
    //predict next plan
...
//---------------------------------------------------------------------------
bool
NVMeTaskFillLba::new_plan_before_issue(IoExPlan *plan)
{
...
//---------------------------------------------------------------------------
//void
//NVMeTaskFillLba::io_ex_post(void)
//{
//}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
NVMeTaskFillLba::NVMeTaskFillLba(uint8_t nthDev, uint8_t nsid, uint64_t slba, uint64_t nlb, uint32_t qDepth, uint64_t timeStamp)
    : NVMeIoEx(nthDev, nsid, slba, nlb, qDepth, timeStamp, "")
{
    rep << LegacyString().sprintf("Fill Disk LBA : slba=0x%lX, nlb=0x%lX\n", slba, nlb);
}
//---------------------------------------------------------------------------
NVMeTaskFillLba::NVMeTaskFillLba(std::string devPath, uint64_t slba, uint64_t nlb, uint32_t qDepth, uint64_t timeStamp)
    : NVMeIoEx(devPath, slba, nlb, qDepth, timeStamp, "")
{
    rep << LegacyString().sprintf("Fill Disk LBA : slba=0x%lX, nlb=0x%lX\n", slba, nlb);
}
//---------------------------------------------------------------------------
NVMeTaskFillLba::~NVMeTaskFillLba(void)
{
